...
@Suite struct FileLockTests {
    ...
    func testFileLockCreation() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "test.lock"
    ...
    func testFileLockConflict() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "conflict.lock"
    ...
    func testFileLockUnlock() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "unlock.lock"
    ...
    func testFileLockReacquisition() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "reacquire.lock"
    ...
    func testWaitForLockImmediate() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "immediate.lock"
            let time = Date()
    ...
    func testWaitForLockTimeout() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "timeout.lock"
    ...
    func testWaitForLockEventualSuccess() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "eventual.lock"
    ...
    func testWithLockSuccess() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "withlock.lock"
    ...
    func testWithLockErrorHandling() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "withlockError.lock"
    ...
    func testWithLockTimeout() async throws {
        try await TemperSwiftTests.withTestHome {
            let lockPath = TemperSwiftTests.ctx.mockedHomeDir! / "withlockTimeout.lock"

            // Create existing lock
            let existingLock = try FileLock(at: lockPath)

            await #expect(throws: TemperSwiftError.self) {
