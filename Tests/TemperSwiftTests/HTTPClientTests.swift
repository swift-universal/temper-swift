...
@Suite(.serialized) struct HTTPClientTests {
    @Test(.tags(.large)) func getSwiftOrgGPGKeys() async throws {
        ...
        try await fs.withTemporary(files: tmpFile) {
            let httpClient = TemperSwiftHTTPClient(httpRequestExecutor: HTTPRequestExecutorImpl())
    ...
    @Test(
    ...
    ) func getTemperSwiftLinuxReleases(url: String) async throws {
        let tmpFile = fs.mktemp()
        try await fs.create(file: tmpFile, contents: nil)
        let tmpFileSignature = fs.mktemp(ext: ".sig")
        try await fs.create(file: tmpFileSignature, contents: nil)
        ...
        try await fs.withTemporary(files: tmpFile, tmpFileSignature, keysFile) {
            ...
            try await retry {
                try await httpClient.getTemperSwiftRelease(url: swiftlyURL).download(to: tmpFile)
            }

            try await retry {
                try await httpClient.getTemperSwiftReleaseSignature(url: swiftlyURL.appendingPathExtension("sig")).download(to: tmpFileSignature)
            }
    ...
    ) func getTemperSwiftMacOSReleases(url: String) async throws {
        let tmpFile = fs.mktemp()
        try await fs.create(file: tmpFile, contents: nil)
        let tmpFileSignature = fs.mktemp(ext: ".sig")
        try await fs.create(file: tmpFileSignature, contents: nil)
    ...
    @Test(.tags(.large)) func getTemperSwiftReleaseMetadataFromSwiftOrg() async throws {
        let httpClient = TemperSwiftHTTPClient(httpRequestExecutor: HTTPRequestExecutorImpl())
        do {
            let currentRelease = try await httpClient.getCurrentTemperSwiftRelease()
    ...
    struct ToolchainSpecifier {
        ...
        let architectures: [TemperSwiftWebsiteAPI.Components.Schemas.Architecture]
        let branches: [ToolchainVersion.Snapshot.Branch]

        init(
            platform: PlatformDefinition,
            architectures: [TemperSwiftWebsiteAPI.Components.Schemas.Architecture] = [.x8664, .aarch64],
    ...
    @Test(
        ...
        guard case let pd = try await TemperSwift.currentPlatform.detectPlatform(TemperSwiftTests.ctx, disableConfirmation: true, platform: nil), pd != PlatformDefinition.rhel9 && pd != PlatformDefinition.ubuntu2004 else {
            return
        }
