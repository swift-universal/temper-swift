...
@Suite struct ListTests {
    ...
    private static let swiftlyVersion = TemperSwiftVersion(major: TemperSwiftCore.version.major, minor: 0, patch: 0)
    ...
    func runListTest(f: () async throws -> Void) async throws {
        try await TemperSwiftTests.withTestHome(name: Self.homeName) {
            try await TemperSwiftTests.withMockedTemperSwiftVersion(latestTemperSwiftVersion: Self.swiftlyVersion) {
                for toolchain in Set<ToolchainVersion>.allToolchains() {
                    try await TemperSwiftTests.installMockedToolchain(toolchain: toolchain)
                }

                try await TemperSwiftTests.runCommand(Use.self, ["use", "latest"])
    ...
    func runList(selector: String?) async throws -> [ToolchainVersion] {
        ...
        let output = try await TemperSwiftTests.runWithMockedIO(List.self, args)
        ...
        guard parsedToolchains.count == lines.filter({ $0.hasPrefix("Swift") || $0.contains("-snapshot") || $0.hasPrefix("xcode") }).count else {
            throw TemperSwiftTestError(message: "unexpected listed toolchains in \(output)")
        }
    ...
    @Test func listInUse() async throws {
        func inUseTest(toolchain: ToolchainVersion, selector: String?) async throws {
            try await TemperSwiftTests.runCommand(Use.self, ["use", toolchain.name])
            ...
            let output = try await TemperSwiftTests.runWithMockedIO(List.self, listArgs)
            let lines = output.flatMap { $0.split(separator: "\n").map(String.init) }
    ...
    @Test func listJsonFormatInUse() async throws {
        try await self.runListTest {
            try await TemperSwiftTests.runCommand(Use.self, ["use", ToolchainVersion.newStable.name])
