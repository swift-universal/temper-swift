...
@Suite struct UninstallTests {
    ...
    @Test(.mockHomeToolchains(Self.homeName, toolchains: []), .mockedTemperSwiftVersion()) func uninstallNoInstalledToolchains() async throws {
        try await #require(throws: Uninstall.UninstallCancelledError.self) {
            _ = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", "1.2.3"], input: ["y"])
        }

        try await TemperSwiftTests.validateInstalledToolchains(
            [],
    ...
    @Test(.mockedTemperSwiftVersion()) func uninstallLatest() async throws {
        let toolchains = Set<ToolchainVersion>.allToolchains().filter { $0.asStableRelease != nil }
        try await TemperSwiftTests.withMockedHome(homeName: Self.homeName, toolchains: toolchains) {
            var installed = toolchains

            for i in 0..<toolchains.count {
                _ = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", "latest"], input: ["y"])
            ...
            try await #require(throws: Uninstall.UninstallCancelledError.self) {
                try await TemperSwiftTests.runCommand(Uninstall.self, ["uninstall", "latest"])
            }
        }
    }

    /// Tests that a fully-qualified stable release version can be supplied to `swiftly uninstall`.
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName)) func uninstallStableRelease() async throws {
        ...
        for toolchain in Set<ToolchainVersion>.allToolchains().filter({ $0.isStableRelease() }) {
            _ = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", toolchain.name], input: ["y"])
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName)) func uninstallSnapshot() async throws {
        var installed: Set<ToolchainVersion> = .allToolchains()
        ...
        try await #require(throws: Uninstall.UninstallCancelledError.self) {
            _ = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", "main-snapshot-2022-01-01"], input: ["y"])
        }
    ...
    @Test(.mockedTemperSwiftVersion()) func bulkUninstall() async throws {
        let toolchains = Set(
            [
        ...
        func bulkUninstallTest(
            ...
            let output = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", argument], input: ["y"])
    ...
    @Test(.mockedTemperSwiftVersion()) func uninstallInUse() async throws {
        ...
        func uninstallInUseTest(
            ...
            let output = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", toRemove.name], input: ["y"])
            ...
            try await TemperSwiftTests.validateInUse(expected: expectedInUse)
        ...
        try await TemperSwiftTests.withMockedHome(homeName: Self.homeName, toolchains: toolchains, inUse: .oldStable) {
            var installed = toolchains
            ...
            try await TemperSwiftTests.runCommand(Use.self, ["use", ToolchainVersion.oldMainSnapshot.name])
            ...
            try await TemperSwiftTests.runCommand(Use.self, ["use", ToolchainVersion.oldReleaseSnapshot.name])
            try await uninstallInUseTest(
                &installed,
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable])) func uninstallLastToolchain() async throws {
        _ = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", ToolchainVersion.oldStable.name], input: ["y"])
        ...
        let symlinks = try await fs.ls(
            atPath: TemperSwift.currentPlatform.swiftlyBinDir(TemperSwiftTests.ctx)
        )
        #expect(symlinks == [])
    }

    /// Tests that aborting an uninstall works correctly.
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: .allToolchains(), inUse: .oldStable)) func uninstallAbort() async throws {
        let preConfig = try await Config.load()
        try await #require(throws: Uninstall.UninstallCancelledError.self) {
            _ = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", ToolchainVersion.oldStable.name], input: ["n"])
        }
        try await TemperSwiftTests.validateInstalledToolchains(
            .allToolchains(),
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable, .newStable])) func uninstallAssumeYes() async throws {
        try await TemperSwiftTests.runCommand(Uninstall.self, ["uninstall", "-y", ToolchainVersion.oldStable.name])
        try await TemperSwiftTests.validateInstalledToolchains(
            [.newStable],
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable, .newStable, .newMainSnapshot, .oldReleaseSnapshot])) func uninstallAll() async throws {
        try await TemperSwiftTests.runCommand(Uninstall.self, ["uninstall", "-y", "all"])
        try await TemperSwiftTests.validateInstalledToolchains(
            [],
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable, .newStable, .newMainSnapshot, .oldReleaseSnapshot])) func uninstallNotInstalled() async throws {
        var config = try await Config.load()
        config.inUse = .newMainSnapshot
        config.installedToolchains.remove(.newMainSnapshot)
        try config.save()

        try await TemperSwiftTests.runCommand(Uninstall.self, ["uninstall", "-y", ToolchainVersion.newMainSnapshot.name])
        try await TemperSwiftTests.validateInstalledToolchains(
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [])) func uninstallXcode() async throws {
        let output = try await TemperSwiftTests.runWithMockedIO(Uninstall.self, ["uninstall", "-y", ToolchainVersion.xcodeVersion.name], throws: Uninstall.UninstallCancelledError.self)
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable, .newStable, .oldMainSnapshot, .newMainSnapshot]))
    ...
    @Test(.mockedTemperSwiftVersion())
    func uninstallOverlappingSelectors() async throws {
        // Set up test with stable releases that can overlap
        try await TemperSwiftTests.withMockedHome(homeName: Self.homeName, toolchains: [.oldStable, .oldStableNewPatch]) {
            let output = try await TemperSwiftTests.runWithMockedIO(
                Uninstall.self,
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable, .newStable, .oldMainSnapshot]))
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable, .newStable]))
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable]))
    ...
    @Test(.mockedTemperSwiftVersion(), .mockHomeToolchains(Self.homeName, toolchains: [.oldStable, .newStable, .oldMainSnapshot], inUse: .oldStable))
    func uninstallMultipleSelectorsInUse() async throws {
        ...
        try await TemperSwiftTests.validateInUse(expected: .newStable)
