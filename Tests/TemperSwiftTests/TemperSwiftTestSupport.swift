...
struct TemperSwiftTestError: LocalizedError {
    let message: String
}
...
let unmockedMsg = "All swiftly test case logic must be mocked in order to prevent mutation of the system running the test. This test must either run swiftly components inside a TemperSwiftTests.with... closure, or it must have one of the @Test traits, such as @Test(.testHome), or @Test(.mock...)"
...
struct HTTPRequestExecutorFail: HTTPRequestExecutor {
    func getCurrentTemperSwiftRelease() async throws -> TemperSwiftWebsiteAPI.Components.Schemas.TemperSwiftRelease { fatalError(unmockedMsg) }
    func getReleaseToolchains() async throws -> [Components.Schemas.Release] { fatalError(unmockedMsg) }
    func getSnapshotToolchains(branch _: TemperSwiftWebsiteAPI.Components.Schemas.SourceBranch, platform _: TemperSwiftWebsiteAPI.Components.Schemas.PlatformIdentifier) async throws -> TemperSwiftWebsiteAPI.Components.Schemas.DevToolchains { fatalError(unmockedMsg) }
    func getGpgKeys() async throws -> OpenAPIRuntime.HTTPBody { fatalError(unmockedMsg) }
    func getTemperSwiftRelease(url _: URL) async throws -> OpenAPIRuntime.HTTPBody { fatalError(unmockedMsg) }
    func getTemperSwiftReleaseSignature(url _: URL) async throws -> OpenAPIRuntime.HTTPBody { fatalError(unmockedMsg) }
...
// Convenience extensions to common TemperSwift and TemperSwiftCore types to set the correct context

extension Config {
    public static func load() async throws -> Config {
        try await Config.load(TemperSwiftTests.ctx)
    }

    public func save() throws {
        try self.save(TemperSwiftTests.ctx)
    }
}

extension TemperSwiftCoreContext {
    public init(
        ...
        self.init(httpClient: TemperSwiftHTTPClient(httpRequestExecutor: httpRequestExecutor))

        self.mockedHomeDir = mockedHomeDir
        self.currentDirectory = mockedHomeDir ?? fs.cwd
        self.httpClient = TemperSwiftHTTPClient(httpRequestExecutor: httpRequestExecutor)
...
struct TestHomeTrait: TestTrait, TestScoping {
    ...
    func provideScope(for _: Test, testCase _: Test.Case?, performing function: @Sendable () async throws -> Void) async throws {
        try await TemperSwiftTests.withTestHome(name: self.name) {
            try await function()
        }
    }
}
...
// extension Trait for mockedTemperSwiftVersion
struct MockedTemperSwiftVersionTrait: TestTrait, TestScoping {
    var name: String = "testHome"

    init(_ name: String) { self.name = name }

    func provideScope(for _: Test, testCase _: Test.Case?, performing function: @Sendable () async throws -> Void) async throws {
        try await TemperSwiftTests.withMockedTemperSwiftVersion(latestTemperSwiftVersion: TemperSwiftVersion(major: TemperSwiftCore.version.major, minor: 0, patch: 0)) {
            try await function()
        }
    }
}

extension Trait where Self == MockedTemperSwiftVersionTrait {
    static func mockedTemperSwiftVersion(_ name: String = "testHome") -> Self { Self(name) }
}
...
struct MockHomeToolchainsTrait: TestTrait, TestScoping {
    ...
    func provideScope(for _: Test, testCase _: Test.Case?, performing function: @Sendable () async throws -> Void) async throws {
        try await TemperSwiftTests.withMockedHome(homeName: self.name, toolchains: self.toolchains, inUse: self.inUse) {
            try await function()
        }
    }
}
...
struct TestHomeMockedToolchainTrait: TestTrait, TestScoping {
    ...
    func provideScope(for _: Test, testCase _: Test.Case?, performing function: @Sendable () async throws -> Void) async throws {
        try await TemperSwiftTests.withTestHome(name: self.name) {
            try await TemperSwiftTests.withMockedToolchain {
                try await function()
            }
        }
    }
...
public enum TemperSwiftTests {
    @TaskLocal static var ctx: TemperSwiftCoreContext = .init(
    ...
    static func baseTestConfig() async throws -> Config {
        guard let pd = try? await TemperSwift.currentPlatform.detectPlatform(Self.ctx, disableConfirmation: true, platform: nil) else {
            throw TemperSwiftTestError(message: "Unable to detect the current platform.")
        }
        ...
        return Config(
            ...
            installedToolchains: [],
            platform: pd,
            version: TemperSwiftCore.version
        )
    }

    static func runCommand<T: TemperSwiftCommand>(_ commandType: T.Type, _ arguments: [String]) async throws {
        let rawCmd = try TemperSwiftCommands.parseAsRoot(arguments)

        guard var cmd = rawCmd as? T else {
            throw TemperSwiftTestError(
    ...
    /// output lines.
    static func runWithMockedIO<T: TemperSwiftCommand, E: Error>(_ commandType: T.Type, _ arguments: [String], quiet: Bool = false, input: [String]? = nil, format: TemperSwiftCore.OutputFormat = .text, throws expectedError: E.Type = NoError.self) async throws -> [String] {
        ...
        let ctx = TemperSwiftCoreContext(
            mockedHomeDir: TemperSwiftTests.ctx.mockedHomeDir,
            httpRequestExecutor: TemperSwiftTests.ctx.httpClient.httpRequestExecutor,
    ...
    static func withTestHome(
        ...
        try await fs.withTemporary(files: [testHome] + TemperSwift.requiredDirectories(ctx)) {
            for dir in TemperSwift.requiredDirectories(ctx) {
    ...
    static func withMockedHome(
        ...
        try await Self.withTestHome(name: homeName) {
            ...
            if !toolchains.isEmpty {
                ...
                try await fs.mkdir(.parents, atPath: TemperSwift.currentPlatform.swiftlyBinDir(Self.ctx))
                cleanBinDir = true
            }
            ...
            do {
                ...
                if cleanBinDir {
                    try await fs.remove(atPath: TemperSwift.currentPlatform.swiftlyBinDir(Self.ctx))
                }
            } catch {
                if cleanBinDir {
                    try await fs.remove(atPath: TemperSwift.currentPlatform.swiftlyBinDir(Self.ctx))
    ...
    static func withMockedTemperSwiftVersion(latestTemperSwiftVersion: TemperSwiftVersion = TemperSwiftCore.version, _ f: () async throws -> Void) async throws {
        let mockDownloader = MockToolchainDownloader(executables: ["swift"], latestTemperSwiftVersion: latestTemperSwiftVersion)

        let ctx = TemperSwiftCoreContext(
            mockedHomeDir: TemperSwiftTests.ctx.mockedHomeDir,
            httpRequestExecutor: mockDownloader,
            outputHandler: TemperSwiftTests.ctx.outputHandler,
            inputProvider: TemperSwiftTests.ctx.inputProvider
        )
    ...
    static func validateInstalledToolchains(_ toolchains: Set<ToolchainVersion>, description: String) async throws {
        ...
        guard config.installedToolchains == toolchains else {
            throw TemperSwiftTestError(message: "\(description): expected \(toolchains) but got \(config.installedToolchains)")
        }
        ...
        for toolchain in toolchains {
            let toolchainDir = TemperSwift.currentPlatform.swiftlyHomeDir(Self.ctx) / "toolchains/\(toolchain.name)"
            #expect(try await fs.exists(atPath: toolchainDir))
    ...
    static func getMockedToolchainVersion(at path: FilePath) throws -> ToolchainVersion {
        ...
        guard let outputData = try outputPipe.fileHandleForReading.readToEnd() else {
            throw TemperSwiftTestError(message: "got no output from swift binary at path \(path)")
        }
...
public actor TestOutputHandler: TemperSwiftCore.OutputHandler {
...
public actor TestInputProvider: TemperSwiftCore.InputProvider {
...
public struct SwiftExecutable {
    ...
    public func version() async throws -> ToolchainVersion {
        ...
        guard let outputData = try outputPipe.fileHandleForReading.readToEnd() else {
            throw TemperSwiftTestError(message: "got no output from swift binary at path \(self.path)")
        }
        ...
        if let match = try Self.stableRegex().firstMatch(in: outputString) {
            ...
            throw TemperSwiftTestError(message: "bad version: \(outputString)")
        }
    }
}
...
public final actor MockToolchainDownloader: HTTPRequestExecutor {
    ...
    private let latestTemperSwiftVersion: TemperSwiftVersion
    ...
    public init(
        ...
        latestTemperSwiftVersion: TemperSwiftVersion = TemperSwiftCore.version,
        ...
        self.signatures = [:]
#endif
        self.latestTemperSwiftVersion = latestTemperSwiftVersion
        self.releaseToolchains = releaseToolchains
        self.snapshotToolchains = snapshotToolchains
    }

    public func getCurrentTemperSwiftRelease() async throws -> TemperSwiftWebsiteAPI.Components.Schemas.TemperSwiftRelease {
        let release = TemperSwiftWebsiteAPI.Components.Schemas.TemperSwiftRelease(
            version: self.latestTemperSwiftVersion.description,
            platforms: [
    ...
    public func getReleaseToolchains() async throws -> [Components.Schemas.Release] {
        let currentPlatform = try await TemperSwift.currentPlatform.detectPlatform(TemperSwiftTests.ctx, disableConfirmation: true, platform: nil)
        ...
        guard let platformName else {
            throw TemperSwiftTestError(message: "Could not detect the current platform in test: \(currentPlatform)")
        }

        return self.releaseToolchains.map { releaseToolchain in
            TemperSwiftWebsiteAPI.Components.Schemas.Release(
    ...
    public func getSnapshotToolchains(branch: TemperSwiftWebsiteAPI.Components.Schemas.SourceBranch, platform _: TemperSwiftWebsiteAPI.Components.Schemas.PlatformIdentifier) async throws -> TemperSwiftWebsiteAPI.Components.Schemas.DevToolchains {
        let currentPlatform = try await TemperSwift.currentPlatform.detectPlatform(TemperSwiftTests.ctx, disableConfirmation: true, platform: nil)
        ...
        let devToolchainsForArch = releasesForBranch.map { branchSnapshot in
            TemperSwiftWebsiteAPI.Components.Schemas.DevToolchainForArch(
                name: TemperSwiftWebsiteAPI.Components.Schemas.DevToolchainKind?.none,
                date: "",
        ...
        if currentPlatform == PlatformDefinition.macOS {
            return TemperSwiftWebsiteAPI.Components.Schemas.DevToolchains(universal: devToolchainsForArch)
        } else if cpuArch == TemperSwiftWebsiteAPI.Components.Schemas.Architecture.x8664 {
            return TemperSwiftWebsiteAPI.Components.Schemas.DevToolchains(x8664: devToolchainsForArch)
        } else if cpuArch == TemperSwiftWebsiteAPI.Components.Schemas.Architecture.aarch64 {
            return TemperSwiftWebsiteAPI.Components.Schemas.DevToolchains(aarch64: devToolchainsForArch)
        } else {
            return TemperSwiftWebsiteAPI.Components.Schemas.DevToolchains()
        }
    }
    ...
    private func makeToolchainDownloadResponse(from url: URL) async throws -> OpenAPIRuntime.HTTPBody {
        ...
        if let match = try Self.releaseURLRegex().firstMatch(in: url.path) {
            ...
            guard case let .snapshot(branch, date) = selector else {
                throw TemperSwiftTestError(message: "unexpected selector: \(selector)")
            }
            toolchain = .init(snapshotBranch: branch, date: date!)
        } else {
            throw TemperSwiftTestError(message: "invalid toolchain download URL: \(url.path)")
        }
    ...
    public func getTemperSwiftRelease(url: URL) async throws -> OpenAPIRuntime.HTTPBody {
        let mockedTemperSwift = try await self.makeMockedTemperSwift(from: url)

        return HTTPBody(Array(mockedTemperSwift))
    }
...
#if os(Linux)
    public func makeMockedTemperSwift(from url: URL) async throws -> Data {
        ...
        if url.path.hasSuffix(".sig") {
            ...
            guard let signature = self.signatures["swiftly"] else {
                throw TemperSwiftTestError(message: "swiftly signature wasn't found in the cache")
            }
        ...
        export GNUPGHOME="\(TemperSwiftTests.ctx.mockedHomeDir!)/.gnupg"
        gpg --batch --import \(gpgKeyFile) >/dev/null 2>&1 || echo -n
        """]
        try importKey.run()
        importKey.waitUntilExit()
        if importKey.terminationStatus != 0 {
            throw TemperSwiftTestError(message: "unable to import test gpg signing key")
        }
        ...
        if detachSign.terminationStatus != 0 {
            throw TemperSwiftTestError(message: "unable to sign archive using the test user's gpg key")
        }
    ...
    public func makeMockedToolchain(toolchain: ToolchainVersion, name: String) async throws -> Data {
        ...
        if name.hasSuffix(".sig") {
            ...
            guard let signature = self.signatures[toolchain.name] else {
                throw TemperSwiftTestError(message: "signature wasn't found in the cache")
            }
...
#elseif os(Windows)
    public func makeMockedTemperSwift(from _: URL) async throws -> Data {
        throw TemperSwiftTestError(
            message: "Windows installer fixtures require an Authenticode-signed executable."
        )
    }

    public func makeMockedToolchain(toolchain _: ToolchainVersion, name _: String) async throws -> Data {
        throw TemperSwiftTestError(
            message: "Windows toolchain fixtures require a real Swift installer executable."
        )
    }
#elseif os(macOS)
    public func makeMockedTemperSwift(from _: URL) async throws -> Data {
        ...
        try await sys.pkgbuild(
            ...
            .version("\(self.latestTemperSwiftVersion)"),
