...
@Suite struct UpdateTests {
    ...
    @Test(.testHomeMockedToolchain()) func updateLatest() async throws {
        try await TemperSwiftTests.installMockedToolchain(selector: .latest)

        let beforeUpdateConfig = try await Config.load()

        try await TemperSwiftTests.runCommand(Update.self, ["update", "latest", "--no-verify", "--post-install-file=\(fs.mktemp())"])
    ...
    @Test(.testHomeMockedToolchain()) func updateLatestToLatest() async throws {
        try await TemperSwiftTests.installMockedToolchain(selector: .stable(major: 5, minor: 9, patch: 0))
        try await TemperSwiftTests.runCommand(Update.self, ["update", "-y", "latest", "--no-verify", "--post-install-file=\(fs.mktemp())"])
    ...
    @Test(.testHomeMockedToolchain()) func updateToLatestMinor() async throws {
        try await TemperSwiftTests.installMockedToolchain(selector: .stable(major: 5, minor: 9, patch: 0))
        try await TemperSwiftTests.runCommand(Update.self, ["update", "-y", "5", "--no-verify", "--post-install-file=\(fs.mktemp())"])
    ...
    @Test(.testHomeMockedToolchain()) func updateToLatestPatch() async throws {
        try await TemperSwiftTests.installMockedToolchain(selector: "5.9.0")

        try await TemperSwiftTests.runCommand(Update.self, ["update", "-y", "5.9.0", "--no-verify", "--post-install-file=\(fs.mktemp())"])
    ...
    @Test(.testHomeMockedToolchain()) func updateGlobalDefault() async throws {
        try await TemperSwiftTests.installMockedToolchain(selector: "6.0.0")

        try await TemperSwiftTests.runCommand(Update.self, ["update", "-y", "--no-verify", "--post-install-file=\(fs.mktemp())"])
        ...
        try await TemperSwiftTests.validateInUse(expected: config.inUse!)
    }
    ...
    @Test(
        ...
        try await TemperSwiftTests.installMockedToolchain(selector: .snapshot(branch: branch, date: date))
    ...
    @Test(.testHomeMockedToolchain()) func updateSelectsLatestMatchingStableRelease() async throws {
        try await TemperSwiftTests.installMockedToolchain(selector: "6.0.1")
        try await TemperSwiftTests.installMockedToolchain(selector: "6.0.0")

        try await TemperSwiftTests.runCommand(Update.self, ["update", "-y", "6.0", "--no-verify", "--post-install-file=\(fs.mktemp())"])
    ...
    @Test(
        ...
        try await TemperSwiftTests.withMockedToolchain {
            try await TemperSwiftTests.installMockedToolchain(selector: .snapshot(branch: branch, date: "2024-06-19"))
            try await TemperSwiftTests.installMockedToolchain(selector: .snapshot(branch: branch, date: "2024-06-18"))
