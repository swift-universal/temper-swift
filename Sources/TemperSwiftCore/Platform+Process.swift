...
extension Platform {
    ...
    public func proxyEnvironment(_ ctx: TemperSwiftCoreContext, env: Environment, toolchain: ToolchainVersion) async throws -> Environment {
        var environment = env
        ...
        let pathSeparator: Character = ":"
        #endif
        var pathComponents = path.split(separator: pathSeparator).map { String($0) }
        ...
        let joinedPath = pathComponents.joined(separator: String(pathSeparator))
        environment = environment.updating(["PATH": joinedPath])
        environment = try await self.updateEnvironmentWithToolchain(ctx, environment, toolchain, path: joinedPath)
