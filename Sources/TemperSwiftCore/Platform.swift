...
public protocol Platform: Sendable {
    ...
    /// directory.
    var defaultTemperSwiftHomeDir: FilePath { get }
    ...
    func swiftlyBinDir(_ ctx: TemperSwiftCoreContext) -> FilePath

    /// The "toolchains" subdirectory that contains the Swift toolchains managed by swiftly.
    func swiftlyToolchainsDir(_ ctx: TemperSwiftCoreContext) -> FilePath
    ...
    func install(_ ctx: TemperSwiftCoreContext, from: FilePath, version: ToolchainVersion, verbose: Bool)
        async throws

    /// Extract swiftly from the provided downloaded archive and install
    /// ourselves from that.
    func extractTemperSwiftAndInstall(_ ctx: TemperSwiftCoreContext, from archive: FilePath) async throws
    ...
    func uninstall(_ ctx: TemperSwiftCoreContext, _ version: ToolchainVersion, verbose: Bool) async throws
    ...
    func verifyTemperSwiftSystemPrerequisites() async throws
    ...
    func verifySystemPrerequisitesForInstall(
        _ ctx: TemperSwiftCoreContext, platformName: String, version: ToolchainVersion,
    ...
    func verifyToolchainSignature(
        _ ctx: TemperSwiftCoreContext, toolchainFile: ToolchainFile, archive: FilePath, verbose: Bool
    )
        async throws

    /// Downloads the signature file associated with the archive and verifies it matches the downloaded archive.
    /// Throws an error if the signature does not match.
    func verifyTemperSwiftSignature(
        _ ctx: TemperSwiftCoreContext, archiveDownloadURL: URL, archive: FilePath, verbose: Bool
    ) async throws

    /// Detect the platform definition for this platform.
    func detectPlatform(_ ctx: TemperSwiftCoreContext, disableConfirmation: Bool, platform: String?)
    ...
    func findToolchainLocation(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion) async throws -> FilePath

    /// Find the location of the toolchain binaries.
    func findToolchainBinDir(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion) async throws -> FilePath

    /// Update the process environment used when proxying based on the selected toolchain.
    func updateEnvironmentWithToolchain(_ ctx: TemperSwiftCoreContext, _ environment: Environment, _ toolchain: ToolchainVersion, path: String) async throws -> Environment
}
...
extension Platform {
    ...
    /// ```
    ///
    public func swiftlyHomeDir(_ ctx: TemperSwiftCoreContext) -> FilePath {
        ctx.mockedHomeDir
            ?? ProcessInfo.processInfo.environment["SWIFTLY_HOME_DIR"].map { FilePath($0) }
            ?? self.defaultTemperSwiftHomeDir
    }

    /// The path of the configuration file in swiftly's home directory.
    public func swiftlyConfigFile(_ ctx: TemperSwiftCoreContext) -> FilePath {
    ...
    public func installTemperSwiftBin(_ ctx: TemperSwiftCoreContext) async throws {
        ...
        guard
            case let swiftlyHomeBin = self.swiftlyBinDir(ctx) / swiftlyExecutableName,
        ...
        do {
            ...
            await ctx.message(
                "TemperSwift has been copied into the installation directory. You can remove '\(cmdAbsolute)'. It is no longer needed."
            )
        }
    }

    // Find the location where swiftly should be executed.
    public func findTemperSwiftBin(_ ctx: TemperSwiftCoreContext) async throws -> FilePath? {
        let swiftlyHomeBin = self.swiftlyBinDir(ctx) / swiftlyExecutableName
        guard let cmdAbsolute = try await self.absoluteCommandPath() else {
            if try await fs.exists(atPath: swiftlyHomeBin) {
    ...
    public func findToolchainBinDir(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion) async throws -> FilePath
    {
    ...
    private func absoluteCommandPath() async throws -> FilePath? {
        ...
        let pathSeparator = ":"
        #endif
        let pathEntries = path.components(separatedBy: pathSeparator).map { FilePath($0) / cmd }
    ...
    private var swiftlyExecutableName: String {
#if os(Windows)
        "temper-swift.exe"
#else
        "temper-swift"
