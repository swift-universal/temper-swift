...
public struct TemperSwiftVersion: Equatable, Comparable, CustomStringConvertible, Sendable {
    ...
    public init(parsing tag: String) throws {
        guard let match = try Self.regex().wholeMatch(in: tag) else {
            throw TemperSwiftError(message: "unable to parse release tag: \"\(tag)\"")
        }
...
extension TemperSwiftVersion: Decodable {
    public init(from decoder: Decoder) throws {
        let c = try decoder.singleValueContainer()
        let s = try c.decode(String.self)
        let v = try TemperSwiftVersion(parsing: s)
        self.major = v.major
        self.minor = v.minor
        self.patch = v.patch
...
extension TemperSwiftVersion: Encodable {
