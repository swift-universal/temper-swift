...
public let version = TemperSwiftVersion(major: 1, minor: 3, patch: 0, suffix: "dev")
...
public struct TemperSwiftCoreContext: Sendable {
    ...
    /// requests.
    public var httpClient: TemperSwiftHTTPClient
    ...
    public init(format: TemperSwiftCore.OutputFormat = .text) {
        self.httpClient = TemperSwiftHTTPClient(httpRequestExecutor: HTTPRequestExecutorImpl())
        self.currentDirectory = fs.cwd
        self.format = format
        self.terminal = SystemTerminal()
    }

    public init(httpClient: TemperSwiftHTTPClient) {
...
#if arch(x86_64)
public let cpuArch = TemperSwiftWebsiteAPI.Components.Schemas.Architecture.x8664
#elseif arch(arm64)
public let cpuArch = TemperSwiftWebsiteAPI.Components.Schemas.Architecture.aarch64
#else
