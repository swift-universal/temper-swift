...
typealias sys = TemperSwiftCore.SystemCommand
...
public struct MacOS: Platform {
    ...
    public func verifyTemperSwiftSystemPrerequisites() throws {
        // All system prerequisites are there for swiftly on macOS
    }

    public func verifySystemPrerequisitesForInstall(
        _ ctx: TemperSwiftCoreContext, platformName _: String, version _: ToolchainVersion,
    ...
    public func install(
        _ ctx: TemperSwiftCoreContext, from tmpFile: FilePath, version: ToolchainVersion, verbose: Bool
    ) async throws {
        guard try await fs.exists(atPath: tmpFile) else {
            throw TemperSwiftError(message: "\(tmpFile) doesn't exist")
        }
    ...
    public func extractTemperSwiftAndInstall(_ ctx: TemperSwiftCoreContext, from archive: FilePath) async throws {
        guard try await fs.exists(atPath: archive) else {
            throw TemperSwiftError(message: "\(archive) doesn't exist")
        }
        ...
        if ctx.mockedHomeDir == nil {
            ...
            guard try await fs.exists(atPath: payload) else {
                throw TemperSwiftError(message: "Payload file could not be found at \(tmpDir).")
            }
    ...
    public func uninstall(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion, verbose: Bool)
        async throws
    {
        if verbose {
    ...
    private func findToolchainBundleID(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion) async throws -> String? {
        ...
        guard let pkgInfo = try? decoder.decode(SwiftPkgInfo.self, from: data) else {
            throw TemperSwiftError(message: "could not decode plist at \(infoPlist)")
        }
    ...
    public func verifyToolchainSignature(
        _: TemperSwiftCoreContext, toolchainFile _: ToolchainFile, archive _: FilePath, verbose _: Bool
    ) async throws {
    ...
    public func verifyTemperSwiftSignature(
        _: TemperSwiftCoreContext, archiveDownloadURL _: URL, archive _: FilePath, verbose _: Bool
    ) async throws {
        // No signature verification is required on macOS since the pkg files have their own signing
        //  mechanism and the swift.org downloadables are trusted by stock macOS installations.
    }
    ...
    public func detectPlatform(
        _: TemperSwiftCoreContext, disableConfirmation _: Bool, platform _: String?
    ...
    public func findToolchainLocation(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion) async throws -> FilePath
    {
    ...
    public func updateEnvironmentWithToolchain(_ ctx: TemperSwiftCoreContext, _ environment: Environment, _ toolchain: ToolchainVersion, path: String) async throws -> Environment {
        ...
        if let existingToolchains = ProcessInfo.processInfo.environment[TOOLCHAINS.rawValue], existingToolchains != bundleID {
            throw TemperSwiftError(message: "You have already set \(TOOLCHAINS.rawValue) environment variable to \(existingToolchains), but swiftly has picked another toolchain. Please unset it or `swiftly use xcode` to use the Xcode selection mechanism.")
        }
           ...
           customToolchainsDir != defaultToolchainsDirectory
        {
            ...
            if !(try await fs.exists(atPath: realCltDir)) {
                throw TemperSwiftError(message: "The macOS command line tools must be installed to support a custom SWIFTLY_TOOLCHAIN_DIR for macOS. You can install it using `xcode-select --install`")
            }
            ...
            {
                throw TemperSwiftError(message: "You have set \(DEVELOPER_DIR.rawValue) environment variable to \(developerDirEnv), but swiftly is trying to use its own location so that a toolchain can be selected. Please unset the environment variable and try again.")
            }
