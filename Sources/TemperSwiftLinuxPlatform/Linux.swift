...
public struct Linux: Platform {
    ...
    public func verifyTemperSwiftSystemPrerequisites() async throws {
        ...
        if !foundTrustedCAs {
            let msg = """
            The ca-certificates package is not installed. TemperSwift won't be able to trust the sites to
            perform its downloads.

            You can install the ca-certificates package on your system to fix this.
            """

            throw TemperSwiftError(message: msg)
        }
    }

    public func verifySystemPrerequisitesForInstall(
        _ ctx: TemperSwiftCoreContext, platformName: String, version _: ToolchainVersion,
    ...
    public func uninstall(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion, verbose _: Bool) async throws {
    ...
    public func verifyToolchainSignature(
        ...
        try await fs.withTemporary(files: sigFile) {
            ...
            do {
                ...
                throw TemperSwiftError(message: "Signature verification failed: \(error).")
            }
        }
    }

    /// Import Swift.org GPG keys for signature verification
    private func importGpgKeys(_ ctx: TemperSwiftCoreContext) async throws {
    ...
    public func verifyTemperSwiftSignature(
        ...
        try await fs.create(file: sigFile, contents: nil)
        try await fs.withTemporary(files: sigFile) {
            try await ctx.httpClient.getTemperSwiftReleaseSignature(
    ...
    private func manualSelectPlatform(_ ctx: TemperSwiftCoreContext, _ platformPretty: String?) async
        -> PlatformDefinition
    {
    ...
    public func detectPlatform(
        _ ctx: TemperSwiftCoreContext, disableConfirmation: Bool, platform: String?
        ...
        guard let releaseFile = releaseFile else {
            ...
            if disableConfirmation {
                throw TemperSwiftError(message: message)
            } else {
    ...
    public func findToolchainLocation(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion) -> FilePath
    {
        self.swiftlyToolchainsDir(ctx) / "\(toolchain.name)"
    }

    public func updateEnvironmentWithToolchain(_: TemperSwiftCoreContext, _ environment: Environment, _: ToolchainVersion, path _: String) async throws -> Environment {
