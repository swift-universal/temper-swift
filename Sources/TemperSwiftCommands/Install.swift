...
struct Install: TemperSwiftCommand {
    ...
    @Argument(
        help: ArgumentHelp(
            ...
            NOTE: TemperSwift downloads toolchains to a temporary file that it later cleans during its installation process. If these files are too big for your system temporary directory, set another location by setting the `TMPDIR` environment variable.
    ...
    var format: TemperSwiftCore.OutputFormat = .text
    ...
    mutating func run() async throws {
        try await self.run(TemperSwift.createDefaultContext(format: self.format))
    }

    private func swiftlyHomeDir(_ ctx: TemperSwiftCoreContext) -> FilePath {
        TemperSwift.currentPlatform.swiftlyHomeDir(ctx)
    }
    ...
    public static func setupProxies(
        ...
        // An embedding host can own the proxy executable while reusing the
        // command implementation. Standalone Temper retains its own binary.
        if let proxyTo = await TemperSwiftCommandInvocation.proxyExecutable(ctx) {
            // Ensure swiftly doesn't overwrite any existing executables without getting confirmation first.
            let swiftlyBinDir = TemperSwift.currentPlatform.swiftlyBinDir(ctx)
            let swiftlyBinDirContents =
                (try? await fs.ls(atPath: swiftlyBinDir)) ?? [String]()
            let toolchainBinDir = try await TemperSwift.currentPlatform.findToolchainBinDir(ctx, version)
            ...
            if !overwrite.isEmpty && !assumeYes {
                ...
                guard await ctx.promptForConfirmation(defaultBehavior: false) else {
                    throw TemperSwiftError(message: "Toolchain installation has been cancelled")
                }
            }
            ...
            for p in proxiesToCreate {
                let proxy = TemperSwift.currentPlatform.swiftlyBinDir(ctx) / p
        ...
        if let version = version {
               ...
               case let .swiftVersionFile(_, sel, error) = result
            {
                if let sel = sel {
                    ...
                    throw TemperSwiftError(message: "Internal error selecting toolchain to install.")
                }
            } else {
                throw TemperSwiftError(
                    message:
                    "TemperSwift couldn't determine the toolchain version to install. Please set a version like this and try again: `swiftly install latest`"
                )
            }
        }
    ...
    public static func execute(
        ...
        let postInstallScript = try await TemperSwift.currentPlatform
        ...
        return try await fs.withTemporary(files: tmpFile) {
            ...
            case .xcode:
                throw TemperSwiftError(message: "xcode toolchain is installed with Xcode")
            }
            ...
            let toolchainFile = ToolchainFile(
                ...
                file:
                "\(version.identifier)-\(platformFullString).\(TemperSwift.currentPlatform.toolchainFileExtension)"
            )
            ...
            if verifySignature {
                try await TemperSwift.currentPlatform.verifyToolchainSignature(
                    ctx,
            ...
            let lockFile = TemperSwift.currentPlatform.swiftlyHomeDir(ctx) / "swiftly.lock"
            if verbose {
            ...
            let (pathChanged, newConfig) = try await withLock(lockFile) {
                ...
                try await TemperSwift.currentPlatform.install(
                    ctx, from: tmpFile,
                    version: version,
                    verbose: verbose
                )
    ...
    public static func resolve(
        _ ctx: TemperSwiftCoreContext, config: Config, selector: ToolchainSelector
    )
        async throws -> ToolchainVersion
    {
        switch selector {
        case .latest:
            ...
            guard
                ...
                throw TemperSwiftError(message: "couldn't get latest releases")
            }
        ...
        case let .stable(major, minor, patch):
            ...
            guard let toolchain else {
                throw TemperSwiftError(message: "No release toolchain found matching \(major).\(minor)")
            }
        ...
        case let .snapshot(branch, date):
            ...
            } catch let branchNotFoundErr as TemperSwiftHTTPClient.SnapshotBranchNotFoundError {
                throw TemperSwiftError(
                    message:
            ...
            guard let firstSnapshot else {
                throw TemperSwiftError(message: "No snapshot toolchain found for branch \(branch)")
            }
