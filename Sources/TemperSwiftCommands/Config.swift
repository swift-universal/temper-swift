...
public struct Config: Codable, Equatable, Sendable {
    ...
    public var version: TemperSwiftVersion

    init(inUse: ToolchainVersion?, installedToolchains: Set<ToolchainVersion>, platform: PlatformDefinition, version: TemperSwiftVersion) {
        self.inUse = inUse
    ...
    public static func load(_ ctx: TemperSwiftCoreContext) async throws -> Config {
        do {
            let configFile = TemperSwift.currentPlatform.swiftlyConfigFile(ctx)
            ...
            let msg = """
            Could not load swiftly's configuration file at \(TemperSwift.currentPlatform.swiftlyConfigFile(ctx)).
    ...
    public func save(_ ctx: TemperSwiftCoreContext) throws {
        let outData = try Self.makeEncoder().encode(self)
        try outData.write(to: TemperSwift.currentPlatform.swiftlyConfigFile(ctx), options: .atomic)
    }

    public func listInstalledToolchains(selector: ToolchainSelector?) -> [ToolchainVersion] {
#if os(macOS)
        let systemToolchains: [ToolchainVersion] = TemperSwiftCommandInvocation.includesSystemToolchains
            ? [.xcodeVersion]
            : []
#else
    ...
    public static func update(_ ctx: TemperSwiftCoreContext, f: (inout Config) throws -> Void) async throws {
