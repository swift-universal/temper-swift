...
struct ListAvailable: TemperSwiftCommand {
    ...
    mutating func run(_ ctx: TemperSwiftCoreContext) async throws {
        ...
        case let .snapshot(branch, _):
            ...
            } catch let branchNotFoundError as TemperSwiftHTTPClient.SnapshotBranchNotFoundError {
                throw TemperSwiftError(message: "The snapshot branch \(branchNotFoundError.branch) was not found on swift.org. Note that snapshot toolchains are only available for the current `main` release and the previous x.y (major.minor) release.")
            } catch {
                throw error
            }
        case .stable, .latest:
