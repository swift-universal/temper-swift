...
struct SelfUninstall: TemperSwiftCommand {
    ...
    mutating func run(_ ctx: TemperSwiftCoreContext) async throws {
        _ = try await validateTemperSwift(ctx)
        let swiftlyBin = TemperSwift.currentPlatform.swiftlyBinDir(ctx)
    ...
    static func execute(_ ctx: TemperSwiftCoreContext, verbose: Bool) async throws {
        await ctx.print("Uninstalling swiftly...")

        let userHome = ctx.mockedHomeDir ?? fs.home
        let swiftlyHome = TemperSwift.currentPlatform.swiftlyHomeDir(ctx)
        let swiftlyBin = TemperSwift.currentPlatform.swiftlyBinDir(ctx)
        ...
        // Remove swiftly symlinks and binary from TemperSwift bin directory
        ...
        if try await fs.exists(atPath: swiftlyBinary) {
            if verbose {
                await ctx.print("TemperSwift binary found at \(swiftlyBinary), removing it...")
            }
        ...
        if entries.isEmpty {
            if verbose {
                await ctx.print("TemperSwift bin directory at \(swiftlyBin) is empty, removing it...")
            }
        ...
        if try await fs.exists(atPath: licenseDir) {
            ...
            if licenseEntries.isEmpty {
                if verbose {
                    await ctx.print("TemperSwift license directory at \(licenseDir) is empty, removing it...")
                }
        ...
        if homeEntries.isEmpty {
            if verbose {
                await ctx.print("TemperSwift home directory at \(swiftlyHome) is empty, removing it...")
            }
            try await fs.remove(atPath: swiftlyHome)
        }

        await ctx.print("TemperSwift is successfully uninstalled.")
    }
}
