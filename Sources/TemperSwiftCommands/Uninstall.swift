...
struct Uninstall: TemperSwiftCommand {
    ...
    private func handleSelectionErrors(_ ctx: TemperSwiftCoreContext, _ result: ToolchainSelectionResult) async throws {
    ...
    private func displayToolchainConfirmation(_ ctx: TemperSwiftCoreContext, _ toolchains: [ToolchainVersion]) async {
    ...
    private func displayProgress(_ ctx: TemperSwiftCoreContext, index: Int, total: Int, toolchain: ToolchainVersion) async {
        if total > 1 {
            await ctx.message("[\(index + 1)/\(total)] Processing \(toolchain)")
        }
    }

    private func displayCompletionMessage(_ ctx: TemperSwiftCoreContext, _ toolchainCount: Int) async {
        await ctx.message()
        if self.toolchains.count == 1 {
    ...
    static func execute(
        _ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion, _ config: inout Config,
        verbose: Bool
    ) async throws {
        ...
        config = try await withLock(lockFile) {
            ...
            try await TemperSwift.currentPlatform.uninstall(ctx, toolchain, verbose: verbose)
            return config
        }
