...
struct Update: TemperSwiftCommand {
    ...
    public mutating func run(_ ctx: TemperSwiftCoreContext) async throws {
        ...
        if let postInstallScript {
            guard let postInstallFile = self.postInstallFile else {
                throw TemperSwiftError(message: """
    ...
    /// parameters.
    private func resolveUpdateParameters(_ ctx: TemperSwiftCoreContext, _ config: inout Config) async throws -> UpdateParameters? {
        ...
        case .xcode:
            throw TemperSwiftError(message: "xcode cannot be updated from swiftly")
        }
    }
    ...
    private func lookupNewToolchain(_ ctx: TemperSwiftCoreContext, _ config: Config, _ bounds: UpdateParameters) async throws -> ToolchainVersion? {
        switch bounds {
