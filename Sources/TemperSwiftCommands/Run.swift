...
struct Run: TemperSwiftCommand {
    ...
    mutating func run(_ ctx: TemperSwiftCoreContext) async throws {
        ...
        if command == ["--version"] {
            throw CleanExit.message(String(describing: TemperSwiftCore.version))
        }
        ...
        if let selector {
            ...
            guard let matchedToolchain else {
                throw TemperSwiftError(message: "The selected toolchain \(selector.description) didn't match any of the installed toolchains. You can install it with `swiftly install \(selector.description)`")
            }
        ...
        guard let toolchain else {
            throw TemperSwiftError(message: "No installed swift toolchain is selected from either from a .swift-version file, or the default. You can try using one that's already installed with `swiftly use <toolchain version>` or install a new toolchain to use with `swiftly install --use <toolchain version>`.")
        }

        let env: Environment = try await TemperSwift.currentPlatform.proxyEnvironment(ctx, env: .inherit, toolchain: toolchain)
        ...
        if try await fs.exists(atPath: commandPath) {
            ...
            let tcBinPath = try await TemperSwift.currentPlatform.findToolchainLocation(ctx, toolchain) / "usr/bin"
    ...
    public static func extractProxyArguments(command: [String]) throws -> (command: [String], selector: ToolchainSelector?) {
        ...
        guard args.command.count > 0 else {
            throw TemperSwiftError(message: "Provide at least one command to run.")
        }
