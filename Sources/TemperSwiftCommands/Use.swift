...
struct Use: TemperSwiftCommand {
    ...
    mutating func run(_ ctx: TemperSwiftCoreContext) async throws {
        ...
        guard let toolchain = self.toolchain else {
            ...
            if self.printLocation {
                let location = LocationInfo(path: "\(try await TemperSwift.currentPlatform.findToolchainLocation(ctx, selectedVersion))")
        ...
        guard !self.printLocation else {
            throw TemperSwiftError(message: "The print location flag cannot be used with a toolchain version.")
        }
        ...
        guard let toolchain = config.listInstalledToolchains(selector: selector).max() else {
            throw TemperSwiftError(message: "No installed toolchains match \"\(toolchain)\"")
        }
    ...
    static func execute(_ ctx: TemperSwiftCoreContext, _ toolchain: ToolchainVersion, globalDefault: Bool, verbose: Bool, assumeYes: Bool = true, _ config: inout Config) async throws -> Bool {
    ...
    static func findNewVersionFile(_ ctx: TemperSwiftCoreContext) async throws -> FilePath? {
...
public func selectToolchain(_ ctx: TemperSwiftCoreContext, config: inout Config, globalDefault: Bool = false) async throws -> (ToolchainVersion?, ToolchainSelectionResult) {
    if !globalDefault {
        var cwd = ctx.currentDirectory
        ...
        while !cwd.isEmpty && !cwd.removingRoot().isEmpty {
            ...
            if try await fs.exists(atPath: svFile) {
                ...
                guard let contents else {
                    return (nil, .swiftVersionFile(svFile, nil, TemperSwiftError(message: "The swift version file could not be read: \(svFile)")))
                }

                guard !contents.isEmpty else {
                    return (nil, .swiftVersionFile(svFile, nil, TemperSwiftError(message: "The swift version file is empty: \(svFile)")))
                }
                ...
                do {
                    ...
                    return (nil, .swiftVersionFile(svFile, nil, TemperSwiftError(message: "The swift version file is malformed: \(svFile) \(error)")))
                }

                guard let selector else {
                    return (nil, .swiftVersionFile(svFile, nil, TemperSwiftError(message: "The swift version file is malformed: \(svFile)")))
                }
                ...
                guard let selectedToolchain = config.listInstalledToolchains(selector: selector).max() else {
                    ...
                    return (nil, .swiftVersionFile(svFile, selector, TemperSwiftError(message: "The swift version file `\(svFile)` requires toolchain version \(selector), which is not installed. You can install it with `swiftly install`, or switch to a different installed toolchain with \(useSuggestion).")))
                }
