...
public enum TemperSwiftVersionMigration {
    case exact(TemperSwiftVersion)
    case minor(TemperSwiftVersion)

    public func matches(_ version: TemperSwiftVersion) -> Bool {
        switch self {
        case let .exact(v):
...
public var migrations: [TemperSwiftVersionMigration] {
    [
...
struct Init: TemperSwiftCommand {
    ...
    mutating func run(_ ctx: TemperSwiftCoreContext = TemperSwift.createDefaultContext()) async throws {
    ...
    static func execute(_ ctx: TemperSwiftCoreContext, assumeYes: Bool, noModifyProfile: Bool, overwrite: Bool, platform: String?, verbose: Bool, skipInstall: Bool, quietShellFollowup: Bool) async throws {
        try await TemperSwift.currentPlatform.verifyTemperSwiftSystemPrerequisites()

        var config = try? await Config.load(ctx)

        func oldEnvSh(_ ctx: TemperSwiftCoreContext) -> String {
            """
            export SWIFTLY_HOME_DIR="\(TemperSwift.currentPlatform.swiftlyHomeDir(ctx))"
            export SWIFTLY_BIN_DIR="\(TemperSwift.currentPlatform.swiftlyBinDir(ctx))"
            export SWIFTLY_TOOLCHAINS_DIR="\(TemperSwift.currentPlatform.swiftlyToolchainsDir(ctx))"
        ...
        func oldEnvFish(_ ctx: TemperSwiftCoreContext) -> String {
            """
            set -x SWIFTLY_HOME_DIR "\(TemperSwift.currentPlatform.swiftlyHomeDir(ctx))"
            set -x SWIFTLY_BIN_DIR "\(TemperSwift.currentPlatform.swiftlyBinDir(ctx))"
            set -x SWIFTLY_TOOLCHAINS_DIR "\(TemperSwift.currentPlatform.swiftlyToolchainsDir(ctx))"
        ...
        func envSh(_ ctx: TemperSwiftCoreContext) -> String {
            """
            export SWIFTLY_HOME_DIR="\(TemperSwift.currentPlatform.swiftlyHomeDir(ctx))"
            export SWIFTLY_BIN_DIR="\(TemperSwift.currentPlatform.swiftlyBinDir(ctx))"
            export SWIFTLY_TOOLCHAINS_DIR="\(TemperSwift.currentPlatform.swiftlyToolchainsDir(ctx))"
        ...
        func envFish(_ ctx: TemperSwiftCoreContext) -> String {
            """
            set -x SWIFTLY_HOME_DIR "\(TemperSwift.currentPlatform.swiftlyHomeDir(ctx))"
            set -x SWIFTLY_BIN_DIR "\(TemperSwift.currentPlatform.swiftlyBinDir(ctx))"
            set -x SWIFTLY_TOOLCHAINS_DIR "\(TemperSwift.currentPlatform.swiftlyToolchainsDir(ctx))"
        ...
        if var config, !overwrite && !migrations.filter({ $0.matches(config.version) }).isEmpty {
            ...
            try await TemperSwift.currentPlatform.installTemperSwiftBin(ctx)

            // Check for an outdated env.sh, and update it if the contents is recognized.
            if case let envFile = (TemperSwift.currentPlatform.swiftlyHomeDir(ctx)) / "env.sh",
            ...
            if case let envFile = (TemperSwift.currentPlatform.swiftlyHomeDir(ctx)) / "env.fish",
               (try? await fs.exists(atPath: envFile)) ?? false,
               let contents = String(data: (try? await fs.cat(atPath: envFile)) ?? Data(), encoding: .utf8),
            ...
            config.version = TemperSwiftCore.version
        ...
        if let config, !overwrite && config.version != TemperSwiftCore.version {
            // We don't support downgrades, and versions prior to 0.4.0-dev
            throw TemperSwiftError(message: "An existing swiftly installation was detected. You can try again with '--overwrite' to overwrite it.")
        }

        // Give the user the prompt and the choice to abort at this point.
        if !assumeYes {
            let toolchainsDir = TemperSwift.currentPlatform.swiftlyToolchainsDir(ctx)
            ...
            TemperSwift installs files into the following locations:

            \(TemperSwift.currentPlatform.swiftlyHomeDir(ctx)) - Directory for configuration files
            \(TemperSwift.currentPlatform.swiftlyBinDir(ctx)) - Links to the binaries of the active toolchain
            ...
            guard await ctx.promptForConfirmation(defaultBehavior: true) else {
                throw TemperSwiftError(message: "swiftly installation has been cancelled")
            }
        }
        ...
        if shell.hasSuffix("fish") {
            envFile = TemperSwift.currentPlatform.swiftlyHomeDir(ctx) / "env.fish"
            sourceLine = """
            ...
            envFile = TemperSwift.currentPlatform.swiftlyHomeDir(ctx) / "env.sh"
            sourceLine = """
        ...
        if overwrite {
#if !os(Windows)
            try? await fs.remove(atPath: TemperSwift.currentPlatform.swiftlyToolchainsDir(ctx))
#endif
            try? await fs.remove(atPath: TemperSwift.currentPlatform.swiftlyHomeDir(ctx))
        }
        ...
        if overwrite || config == nil {
            let pd = try await TemperSwift.currentPlatform.detectPlatform(ctx, disableConfirmation: assumeYes, platform: platform)
            let c = Config(inUse: nil, installedToolchains: [], platform: pd, version: TemperSwiftCore.version)

            try c.save(ctx)
            config = c
        }

        guard var config else { throw TemperSwiftError(message: "Configuration could not be set") }
