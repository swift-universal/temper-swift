...
@main
struct TestTemperSwift: AsyncParsableCommand {
    ...
    mutating func run() async throws {
        ...
        throw ValidationError(
            "test-temper-swift exercises the Unix archive and shell installer and is not available on Windows. Run the Temper Swift package tests instead."
        )
#else
        if !self.assumeYes {
...
#if os(Linux)
        let extractedTemperSwift = FilePath("./swiftly")
#elseif os(macOS)
        let extractedTemperSwift = FilePath((fs.home / ".swiftly/bin/swiftly").string)
#endif
        ...
        if self.customLocation {
            ...
            let config = Configuration(
                executable: .path(extractedTemperSwift),
    ...
    private func verifyCustomLocationCleanup(customLoc: FilePath) async throws {
        ...
        guard !(try await fs.exists(atPath: swiftlyBinary)) else {
            throw TestError("TemperSwift binary still exists at \(swiftlyBinary)")
        }
    ...
    private func verifyProfileCleanup() async throws {
        ...
        for profilePath in profilePaths {
            ...
            {
                throw TestError("TemperSwift references still found in profile file: \(profilePath)")
            }
        }
