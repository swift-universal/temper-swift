...
TemperSwift toolchains include a variety of compilers, linkers, debuggers, documentation generators, and other useful tools for working with Swift. Using a toolchain activates it so that when you run toolchain commands they are run with that version.
...
TemperSwift uses the newest matching version that you have installed, if any.
...
TemperSwift can create and update a special `.swift-version` file at the top of your git repository so that you can share your toolchain preference with the rest of your team:
...
TemperSwift uses the `.swift-version` file, if present in your working directory (or parent), to select the toolchain that is in use.
