...
Install swift toolchains with TemperSwift.
...
TemperSwift provides instructions for any additional dependencies that need to be installed.
...
Once you've installed more than one toolchain you may notice that swift is on the first version that you installed, not the last one. TemperSwift lets you quickly switch between toolchains by "using" them. There's a swiftly subcommand for that.
...
TemperSwift supports installing development snapshot toolchains. For example, you can install the latest available snapshot for the next major release using the "main-snapshot" selector and prepare your code for when it arrives.
